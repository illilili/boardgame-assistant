...
/**
 * 인터랙티브 대시보드 초기 데이터 일괄 조회
 * @returns {Promise<Object>} 초기화에 필요한 모든 데이터
 */
export const fetchInteractiveDashboardInitData = async () => {
  try {
    console.log('🚀 인터랙티브 대시보드 초기 데이터 조회 시작...');
    
    // 인기 카테고리/메카닉과 전체 목록을 병렬 조회
    const [popularCategories, popularMechanics, allCategories, allMechanics, health] = await Promise.all([
      fetchInteractivePopularCategories(),
      fetchInteractivePopularMechanics(),
      fetchInteractiveCategories(),
      fetchInteractiveMechanics(),
      fetchInteractiveHealth().catch(() => ({ status: 'unknown' }))
    ]);
    
    // 기본 필터링된 게임 데이터 (초기 상태)
    const initialGames = await fetchInteractiveFilteredGames({
      complexityMin: 1.0,
      complexityMax: 5.0,
      limit: 500, // 500개로 증가하여 충분한 탐색 다양성 제공
      sortBy: 'geek_rating',
      sortOrder: 'desc'
    });
    
    // 기본 버블 차트 데이터 (초기 게임과 동일한 조건 사용)
    const initialBubbleData = await fetchInteractiveBubbleChart({
      groupBy: 'categories',
      complexityMin: 1.0,
      complexityMax: 5.0,
      players: [],
    ...
    // 기본 히트맵 데이터 (초기 게임과 동일한 조건 사용)
    const initialHeatmapData = await fetchInteractiveHeatmap({
      complexityMin: 1.0,
      complexityMax: 5.0,
      players: [],
      categories: [],
    ...
    const result = {
      // 인기 목록을 기본으로, 전체 목록은 검색용으로 제공
      categories: popularCategories,
      mechanics: popularMechanics,
      allCategories, // 검색 시 전체 목록 사용
      allMechanics,  // 검색 시 전체 목록 사용
      health,
      initialGames,
      initialBubbleData,
      initialHeatmapData,
      lastUpdated: new Date(),
      source: '10K 데이터셋 (인터랙티브)'
    };
    
    console.log('✅ 인터랙티브 대시보드 초기 데이터 조회 완료');
    console.log(`📊 데이터 요약: ${popularCategories.length}개 인기 카테고리 (전체 ${allCategories.length}개), ${popularMechanics.length}개 인기 메카닉 (전체 ${allMechanics.length}개), ${initialGames.length}개 초기 게임`);
    
    return result;
  } catch (error) {
    console.error('인터랙티브 대시보드 초기 데이터 조회 실패:', error);
    throw new Error(`인터랙티브 대시보드 초기화 실패: ${error.message}`);
  }
};
