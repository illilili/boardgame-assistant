...
class GameRuleGenerationResponse(BaseModel):
    ruleId: int = Field(default_factory=lambda: fake.random_int(min=10000, max=99999))
    gameOverview: str
    gameSetup: str
    turnStructure: str
    actionRules: List[str]
    endOfGameTrigger: str
...
class GameRuleRegenerationResponse(BaseModel):
    ruleId: int
    turnStructure: str
    actionRules: List[str]
    victoryCondition: str
...
def generate_rules_api(request: GameRuleGenerationRequest):
    try:
        ...
        if not isinstance(game_rules.get("ruleId"), int):
            game_rules["ruleId"] = fake.random_int(min=10000, max=99999)
